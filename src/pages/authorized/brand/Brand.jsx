...
import { useDeleteBrand } from '@hooks/useMutation';
...
const Brand = () => {
  ...
  /* Queries */
  const { data: brandData, isLoading: isBrandLoading } = useFetchBrand({ page, perPage });
  const brand = brandData?.data?.data || [];
  const meta = brandData?.data?.meta || {};

  /* Mutations */
  const deleteMutation = useDeleteBrand(closeDeleteModal);
  ...
  const handleEdit = (row) => {
    navigate('/dashboard/brand/brand-add', { state: { brand: row, isEdit: true } });
  };
  ...
  const handleRedirect = () => {
    navigate('/dashboard/brand/brand-add');
  };
...
<>
      ...
      <div className='card'>
        <div className='card_header'>
          <div className='top'>
            <div className='left'> 
              <h2>Brands <span>({meta.total || 0})</span></h2>
            </div>
            <div className='right'> 
      ...
      <h2><FiAlertTriangle /> Delete brand</h2>
      <p>Are you sure you want to brand <strong>{deletingBrand?.name}</strong>?</p>
